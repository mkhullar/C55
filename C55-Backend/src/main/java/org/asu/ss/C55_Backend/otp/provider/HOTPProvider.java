...
package org.asu.ss.C55_Backend.otp.provider;
...
import org.asu.ss.C55_Backend.OTPProviderBase;

public class HOTPProvider extends OTPProviderBase {
	...
	public String generate(String key, String base, int digits) {
		...
		if (properties.containsKey(CHECKSUM)) {
			checksum = Boolean.parseBoolean(properties.get(CHECKSUM).toString());
		}

		if (properties.containsKey(TRUNCATE_OFFSET)) {
			truncOffset = Integer.parseInt(properties.get(TRUNCATE_OFFSET).toString());
		}
